...
const data = [
  ...
  { movil: '1389', placa: 'SRF586', conductor: ['WILFREDO AMADO PEÑA', 'DAVID FERNANDO AMADO ARDILA'] },
  ...
  { movil: '1410', placa: 'SYT865', conductor: ['JOSE NUMEN SANTO FONSECA CORONADO', 'JHONATTAN ESTIBEN SANTANA SANCHEZ'] },
  ...
  { movil: '1413', placa: 'SRF511', conductor: 'MILBER FERLEY VILLAMIL CASTELLANOS' },
...
async function main() {
  ...
  for (const item of data) {
    // Evitar upsert porque la DB no tiene índice único en `movil`
    let automovil = await prisma.automovil.findFirst({ where: { movil: item.movil } });
    if (automovil) {
    ...
    for (const nombreConductor of conductores) {
      if (
        nombreConductor &&
        !['NO TIENE CONDUCTOR', 'ADMINISTRACION/NO TIENE CONDUCTOR', 'PROCESO VENTA'].includes(nombreConductor)
      ) {
        let conductor = await prisma.conductor.findFirst({ where: { nombre: nombreConductor } });

        if (!conductor) {
          conductor = await prisma.conductor.create({
            data: { nombre: nombreConductor, cedula: generateRandomCedula() },
          });
        }

        const existingLink = await prisma.conductorAutomovil.findFirst({
          where: { conductorId: conductor.id, automovilId: automovil.id },
        });

        if (!existingLink) {
          await prisma.conductorAutomovil.create({
            data: { conductorId: conductor.id, automovilId: automovil.id },
          });
        }
